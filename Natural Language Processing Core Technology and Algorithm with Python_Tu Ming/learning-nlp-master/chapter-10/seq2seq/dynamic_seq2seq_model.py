...
class dynamicSeq2seq():
    ...
    def __init__(self, encoder_cell, decoder_cell, encoder_vocab_size, decoder_vocab_size, embedding_size,
                 bidirectional=True, attention=False, debug=False, time_major=False):
        self.debug = debug
    ...
    @property       # 装饰器，使方法像属性一样调用，就像是一种特殊的属性
    ...
    def _init_decoder_train_connectors(self):
        # tf.name_scope当传入已存在的name_scope对象时，则其范围内变量的前缀只与当前传入的对象有关，与更上层的name_scope无关
        with tf.name_scope('DecoderTrainFeeds'):
            ...
            # tf.concat拼接的张量只会改变一个维度,其他维度是保存不变的
    ...
    def _init_embeddings(self):
        with tf.variable_scope("embedding") as scope:
            sqrt3 = math.sqrt(3)
            # tf.random_uniform_initializer生成具有均匀分布的张量的初始化器
            ...
            self.decoder_embedding_matrix = tf.get_variable(name="decoder_embedding_matrix",
                                                            ...
                                                            initializer=initializer, dtype=tf.float32)
            # encoder的embedd
            # tf.nn.embedding_lookup选取一个张量里面索引对应的元素
            self.encoder_inputs_embedded = tf.nn.embedding_lookup(self.encoder_embedding_matrix,
                                                                  self.encoder_inputs)
            # decoder的embedd
            self.decoder_train_inputs_embedded = tf.nn.embedding_lookup(self.decoder_embedding_matrix,
    ...
    def _init_bidirectional_encoder(self):
        ...
        with tf.variable_scope("BidirectionalEncoder") as scope:
            ...
            if isinstance(encoder_fw_state, LSTMStateTuple):        # 用来判断一个对象的变量类型
