...
class Seq2seq():
    ...
    def train(self):
        ...
        config.gpu_options.allow_growth = True      # 分配器将不会指定所有的GPU内存，而是根据需求增长
        ...
        with tf.Session(config=config) as sess:
            ...
            for batch in range(self.max_batches + 1):
                ...
                start = time.time()
                fd = self.data_iter(train_src, train_targets, self.batch_size, self.sample_num)
                _, loss, _, _ = sess.run([self.model.train_op, self.model.loss,
                                          self.model.gradient_norms, self.model.updates], fd)
                stop = time.time()
                ...
                if batch == 0 or batch % self.show_epoch == 0:
                    ...
                    self.clearModel()
                    total_time = 0
                    for i, (e_in, dt_pred) in enumerate(zip(fd[self.model.decoder_targets].T,
    ...
    def onlinelearning(self, input_strs, target_strs):
        ...
        target_seg = jieba.lcut(target_strs)    # jieba.lcut 生成List
    ...
    def make_inference_fd(self, inputs_seq):
        ...
        return {self.model.encoder_inputs: inputs_time_major, self.model.encoder_inputs_length: sequence_lengths}
    ...
    def test(self):
        with tf.Session() as sess:
            ...
            for batch in range(self.max_batches + 1):
                # 获取fd [time_steps, batch_size]
                fd = self.data_iter(train_src, train_targets, 2, 3)
