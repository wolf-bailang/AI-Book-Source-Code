...
"""
import numpy as np
from sklearn.cross_validation import train_test_split   # 用于将矩阵随机划分为训练子集和测试子集,并返回划分好的训练集测试集样本和训练集测试集标签
...
def remove_empty_docs(corpus, labels):
    filtered_corpus = []
    filtered_labels = []
    for doc, label in zip(corpus, labels):      # zip()并行遍历
        if doc.strip():     # 移除字符串头尾指定的字符（默认为空格或换行符）或字符序列,只能删除开头或是结尾的字符，不能删除中间部分的字符
...
def main():
    ...
    bow_test_features = bow_vectorizer.transform(norm_test_corpus)      # transform()通过找中心和缩放等实现数据标准化

    # tfidf 特征
    tfidf_vectorizer, tfidf_train_features = tfidf_extractor(norm_train_corpus)
    tfidf_test_features = tfidf_vectorizer.transform(norm_test_corpus)   # 数据标准化

    # tokenize documents 标准文件
    tokenized_train = [jieba.lcut(text) for text in norm_train_corpus]      # 直接返回 list
    ...
    # 训练分类器
    from sklearn.naive_bayes import MultinomialNB       # 朴素贝叶斯分类器，出现次数作为特征值
