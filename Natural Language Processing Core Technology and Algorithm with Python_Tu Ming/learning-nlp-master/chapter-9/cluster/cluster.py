...
vectorizer, feature_matrix = build_feature_matrix(norm_book_content, feature_type='tfidf',
                                                  min_df=0.2, max_df=0.90, ngram_range=(1, 2))
# 查看特征数量
...
def get_cluster_data(clustering_obj, book_data, feature_names, num_clusters, topn_features=10):
    cluster_details = {}
    # 获取cluster的center
    # argsort()函数是将 Numpy 数组 进行升序排列 ，并返回排序后的索引
    # [::-1]运算是将升序变成降序
...
from sklearn.manifold import MDS    # 多维标度法是一种在低维空间展示“距离”数据结构的多元数据分析技术
...
def plot_clusters(num_clusters, feature_matrix, cluster_data, book_data, plot_size=(16, 8)):
    # generate random color for clusters生成随机颜色
    def generate_random_color():
        color = '#%06x' % random.randint(0, 0xFFFFFF)
        return color

    # define markers for clusters 为群集定义标记
    markers = ['o', 'v', '^', '<', '>', '8', 's', 'p', '*', 'h', 'H', 'D', 'd']
    # build cosine distance matrix 建立余弦距离矩阵
    cosine_distance = 1 - cosine_similarity(feature_matrix)
    # dimensionality reduction using MDS 使用MDS减少维度
...
cluster_data = get_cluster_data(clustering_obj=km_obj, book_data=book_data, feature_names=feature_names,
                                num_clusters=num_clusters, topn_features=5)
print_cluster_data(cluster_data)
plot_clusters(num_clusters=num_clusters, feature_matrix=feature_matrix, cluster_data=cluster_data,
              book_data=book_data, plot_size=(16, 8))
...
cluster_data = get_cluster_data(clustering_obj=ap_obj, book_data=book_data,
